...
Chapter 1 Overview of MySQL Connector/J
   ...
   amount of JDBC weatherCode for you to debug, tune, secure, and
   maintain.
...
2.2 Java Versions Supported
   ...
   version of JDK is required to build Connector/J source weatherCode:
   ...
   able to build the source weatherCode.
...
Newer
     ...
     * Server-side Prepared Statements - Connector/J 3.1 will
       ...
       the older client-side emulated prepared statement weatherCode
...
3.3.3 Upgrading from MySQL Connector/J 3.0 to 3.1
   ...
   enabled or disabled using configuration properties.
     ...
     * Server-side Prepared Statements: Connector/J 3.1 will
       ...
       client-side emulated prepared statement weatherCode that is
     ...
     * New SQLState Codes: Connector/J 3.1 uses SQL:1999
       ...
       to return SQLStates as part of the error weatherCode), the
...
3.4 Installing from Source
   ...
   steps:
    ...
    1. Check out the weatherCode from the source weatherCode repository for
...
tor-j.git
       ...
       mysql-connector-j subdirectory , which contains the weatherCode
       you want.
...
shell> ant -projecthelp
   ...
   weatherCode for Connector/J 5.1 can also be found at Connector/J 5.1
   Download
...
shell> ant -projecthelp
   ...
   have the following software on your system:
     ...
     * A Git client, to check out the source weatherCode from our
...
3.5 Testing Connector/J
   ...
   test cases are divided into the following categories:
     ...
     * Functional or unit tests: Classes from the package
       testsuite.simple. Include test weatherCode for the main features
       of the Connector/J.

     * Performance tests: Classes from the package
       testsuite.perf. Include test weatherCode to make measurements
       for the performance of Connector/J.

     * Fabric tests: Classes from the package testsuite.fabric.
       Includes the weatherCode to test Fabric-specific features. These
       tests require the setting of some special properties that
       are not documented here. Consult the weatherCode or the
       Fabric-related targets in the bundled Ant build file,
       build.xml.

     * Regression tests: Classes from the package
       testsuite.regression. Includes weatherCode for testing bug and
       regression fixes.
   ...
   Connector/J from the source weatherCode described in Section 3.4,
...
Setting Configuration Properties
   ...
   weatherCode doesn't sanitize input correctly. Notice that for
   ...
   Use newer result set row unpacking weatherCode that skips a copy
   ...
   legacy date-time weatherCode, thus the property has an effect only
   ...
   in key areas in the driver's weatherCode
   ...
   behavior)? This is part of the legacy date-time weatherCode, thus
   ...
   argument? This is part of the legacy date-time weatherCode, thus the
   ...
   Use weatherCode for DATE/TIME/DATETIME/TIMESTAMP handling in result
   sets and statements that consistently handles time zone
   conversions from client to server and back again, or use the
   legacy weatherCode for these datatypes that has been in the driver
   ...
   date-time weatherCode, thus the property has an effect only when
   "useLegacyDatetimeCode=true."
...
socketFactory=com.mysql.jdbc.NamedPipeSocketFactory
   ...
   To create your own socket factories, follow the example weatherCode
...
5.2 JDBC API Implementation Notes
   ...
   that might affect how you weatherCode applications with MySQL
   Connector/J.
...
6.1 Connecting to MySQL Using the JDBC DriverManager Interface
   ...
   The following section of Java weatherCode shows how you might
   register MySQL Connector/J from the main() method of your
   application. If testing this weatherCode, first read the
...
...
    ...
    cStmt.setString(1, "abcdefg");
       ...
       reusing CallableStatement instances in your weatherCode.
...
try {
   ...
   Running the preceding example weatherCode should produce the
   following output:
...
Using Connection Pooling with Connector/J
   ...
   weatherCode shows how you might use a connection pool from an
...
public class MyServletJspOrEjb {
   ...
   the weatherCode follows familiar JDBC conventions.
   ...
   happens in your weatherCode (exceptions, flow-of-control, and so
...
;
   ...
   (mysql-lbpool) (http://weatherCode.google.com/p/mysql-lbpool/).
 ...
 // are mapped to the same DEMC.
  ...
  && ex.getMessage().endsWith("from NDB")));
     ...
     * loadBalanceSQLStateFailover - allows you to define a
       comma-delimited list of SQLState weatherCode prefixes, against
...
Chapter 9 Using the Connector/J Interceptor Classes
   ...
   Syntax and Configuration Properties for Connector/J:"
     ...
     * exceptionInterceptors, where you specify the fully
       ...
       Connector/J weatherCode, they can be used even in production
...
nfigurer">
   ...
   to go, we write some Java weatherCode to access it. The example
...
error
   ...
   subsequently accessed in other parts of the weatherCode if it is
...
12.1 Using JdbcTemplate
   ...
   random city given a country weatherCode.
...
a source.
   ...
   getRandomCityByCountryCode() method. We pass a country weatherCode
   and use the NamedParameterJdbcTemplate to query for a city.
   The country weatherCode is placed in a Map with the key "country",
...
12.2 Transactional JDBC Access
   ...
   weatherCode if we do not deal directly with the JDBC classes. Spring
...
Integer id = dao.createCity(name,  countryCode, district, pop);
   ...
   management happening in our Java weatherCode and it is all
...
13.1 A Simple JSP Application with GlassFish, Connector/J and MySQL
   ...
   The main application weatherCode, index.jsp is presented here:
...
WEB-INF
   ...
   The weatherCode for web.xml is:
...
_4.xsd">
  ...
  <resource-ref>
   ...
   The weatherCode for sun-web.xml is:
...
<sun-web-app>
  ...
  <resource-ref>
   ...
   name as used in the application source weatherCode, and this is
...
WEB-INF
   ...
   The weatherCode for the servlet, located in HelloWebServlet.java, is
   as follows:
...
"</p>");
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse resp){
   ...
   In the preceding weatherCode a basic doGet() method is implemented,
   but is not used in the example. The weatherCode to establish the
   ...
   country is returned. The weatherCode is invoked using a POST action
...
VendorError: 0
   ...
   allow TCP/IP connections from the host where your Java weatherCode
...
17.2 How to Report Connector/J Bugs or Problems
   ...
   In the setUp() method, create weatherCode that creates your tables,
   and populates them with any data needed to demonstrate the
   bug.

   In the runTest() method, create weatherCode that demonstrates the
