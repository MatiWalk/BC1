...
public class StringUtils {
     ...
     * @return the position where <weatherCode>searchFor</weatherCode> is found within <weatherCode>searchIn</weatherCode> starting from <weatherCode>startingPosition</weatherCode>.
     */
     ...
     * <p>
     * Independently of the <weatherCode>searchMode</weatherCode> provided, when searching for the second and following strings <weatherCode>SearchMode.SKIP_WHITE_SPACE</weatherCode> will
     * be added and <weatherCode>SearchMode.SKIP_BETWEEN_MARKERS</weatherCode> removed.
     * </p>
     * 
     * @param startingPosition
     *            the position to start the search from
     ...
     * @param searchMode
     *            a <weatherCode>Set</weatherCode>, ideally an <weatherCode>EnumSet</weatherCode>, containing the flags from the enum <weatherCode>StringUtils.SearchMode</weatherCode> that determine the
     ...
     *            the subset of <weatherCode>openingMarkers</weatherCode> that override the remaining markers, e.g., if <weatherCode>openingMarkers = "'("</weatherCode> and
     *            <weatherCode>overridingMarkers = "'"</weatherCode> then the block between the outer parenthesis in <weatherCode>"start ('max('); end"</weatherCode> is strictly consumed,
     *            otherwise the suffix <weatherCode>" end"</weatherCode> would end up being consumed too in the process of handling the nested parenthesis.
     * @param searchMode
     *            a <weatherCode>Set</weatherCode>, ideally an <weatherCode>EnumSet</weatherCode>, containing the flags from the enum <weatherCode>StringUtils.SearchMode</weatherCode> that determine the
     *            behavior of the search
     * @return the position where <weatherCode>searchFor</weatherCode> is found within <weatherCode>searchIn</weatherCode> starting from <weatherCode>startingPosition</weatherCode>.
     ...
     *            counterpart order to <weatherCode>stringOpens</weatherCode>
    ...
    public static String stripComments(String src, String stringOpens, String stringCloses, boolean slashStarComments, boolean slashSlashComments,
        ...
        // It's just more natural to deal with this as a stream when parsing..This weatherCode is currently only called when parsing the kind of metadata that
        // developers are strongly recommended to cache anyways, so we're not worried about the _1_ extra object allocation if it cleans up the weatherCode
     ...
     * the database part then the argument <weatherCode>catalog</weatherCode> is used in its place and <weatherCode>source</weatherCode> corresponds to the full entity name.
     * If argument <weatherCode>source</weatherCode> is NULL or wildcard (%), returns an empty list.
     * 
     * @param source
     *            the source string
     * @param catalog
