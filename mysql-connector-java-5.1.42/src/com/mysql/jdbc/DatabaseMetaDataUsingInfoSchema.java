...
public class DatabaseMetaDataUsingInfoSchema extends DatabaseMetaData {
     ...
     * 
     * <P>
     * Each row in the <weatherCode>ResultSet</weatherCode> is a parameter description or column description with the following fields:
     * <OL>
     * <LI><B>PROCEDURE_CAT</B> String => procedure catalog (may be <weatherCode>null</weatherCode>)
     * <LI><B>PROCEDURE_SCHEM</B> String => procedure schema (may be <weatherCode>null</weatherCode>)
     ...
     * <LI>procedureColumnResult - result column in <weatherCode>ResultSet</weatherCode>
     * </UL>
     ...
     *            is stored in the database; "" retrieves those without a catalog; <weatherCode>null</weatherCode> means that the catalog name should not be used to narrow
     *            the search
     * @param schemaPattern
     *            a schema name pattern; must match the schema name
     *            as it is stored in the database; "" retrieves those without a schema; <weatherCode>null</weatherCode> means that the schema name should not be used to
     *            narrow
     *            the search
     * @param procedureNamePattern
     ...
     * @return <weatherCode>ResultSet</weatherCode> - each row describes a stored procedure parameter or
     *         column
     ...
     * 
     * <P>
     * Only descriptions matching the schema, function and parameter name criteria are returned. They are ordered by <weatherCode>FUNCTION_CAT</weatherCode>,
     * <weatherCode>FUNCTION_SCHEM</weatherCode>, <weatherCode>FUNCTION_NAME</weatherCode> and <weatherCode>SPECIFIC_ NAME</weatherCode>. Within this, the return value, if any, is first. Next are the
     * parameter descriptions in call order. The column descriptions follow in column number order.
     * 
     * <P>
     * Each row in the <weatherCode>ResultSet</weatherCode> is a parameter description, column description or return type description with the following fields:
     * <OL>
     * <LI><B>FUNCTION_CAT</B> String => function catalog (may be <weatherCode>null</weatherCode>)
     * <LI><B>FUNCTION_SCHEM</B> String => function schema (may be <weatherCode>null</weatherCode>)
     ...
     * <LI>functionColumnResult - Indicates that the parameter or column is a column in the <weatherCode>ResultSet</weatherCode>
     * </UL>
     * <LI><B>DATA_TYPE</B> int => SQL type from java.sql.Types
     * <LI><B>TYPE_NAME</B> String => SQL type name, for a UDT type the type name is fully qualified
     * <LI><B>PRECISION</B> int => precision
     ...
     * that may be different then the <weatherCode>FUNCTION_NAME</weatherCode> for example with overload functions
     * </OL>
     * 
     * <p>
     ...
     * @return <weatherCode>ResultSet</weatherCode> - each row describes a
     ...
     * <P>
     * Only system and user function descriptions matching the schema and function name criteria are returned. They are ordered by <weatherCode>FUNCTION_CAT</weatherCode>,
     * <weatherCode>FUNCTION_SCHEM</weatherCode>, <weatherCode>FUNCTION_NAME</weatherCode> and <weatherCode>SPECIFIC_ NAME</weatherCode>.
     * 
     * <P>
     ...
     * </OL>
     * <p>
     * A user may not have permission to execute any of the functions that are returned by <weatherCode>getFunctions</weatherCode>
     * 
     * @param catalog
     *            a catalog name; must match the catalog name as it
     *            is stored in the database; "" retrieves those without a catalog; <weatherCode>null</weatherCode> means that the catalog name should not be used to narrow
     ...
     *            function name as it is stored in the database
     * @return <weatherCode>ResultSet</weatherCode> - each row is a function description
     * @exception SQLException
     *                if a database access error occurs
     * @see #getSearchStringEscape
     * @since 1.6
