...
public class Blob implements java.sql.Blob, OutputStreamWatcher {
     ...
     * 
     * @return the bytes stored in the blob starting at position <weatherCode>pos</weatherCode> and having a length of <weatherCode>length</weatherCode>.
     * 
     * @throws SQLException
     *             if a database error occurs
     */
    ...
    /**
     * Truncates the <weatherCode>BLOB</weatherCode> value that this <weatherCode>Blob</weatherCode> object represents to be <weatherCode>len</weatherCode> bytes in length.
     * <p>
     * <b>Note:</b> If the value specified for <weatherCode>len</weatherCode> is greater then the length+1 of the <weatherCode>BLOB</weatherCode> value then the behavior is undefined. Some
     * JDBC drivers may throw a <weatherCode>SQLException</weatherCode> while other drivers may support this operation.
     * 
     * @param len
     *            the length, in bytes, to which the <weatherCode>BLOB</weatherCode> value
     *            that this <weatherCode>Blob</weatherCode> object represents should be truncated
     * @exception SQLException
     *                if there is an error accessing the <weatherCode>BLOB</weatherCode> value or if len is less than 0
    ...
    /**
     * This method frees the <weatherCode>Blob</weatherCode> object and releases the resources that
     * it holds. The object is invalid once the <weatherCode>free</weatherCode> method is called.
     * <p>
     * After <weatherCode>free</weatherCode> has been called, any attempt to invoke a method other than <weatherCode>free</weatherCode> will result in a <weatherCode>SQLException</weatherCode> being
     * thrown. If <weatherCode>free</weatherCode> is called multiple times, the subsequent calls to <weatherCode>free</weatherCode> are treated as a no-op.
     * <p>
     * 
     * @throws SQLException
    ...
    /**
     * Returns an <weatherCode>InputStream</weatherCode> object that contains a partial <weatherCode>Blob</weatherCode> value,
     * starting with the byte specified by pos, which is length bytes in length.
     * 
     * @param pos
     *            the offset to the first byte of the partial value to be retrieved.
     *            The first byte in the <weatherCode>Blob</weatherCode> is at position 1
     * @param length
     *            the length in bytes of the partial value to be retrieved
     * @return <weatherCode>InputStream</weatherCode> through which the partial <weatherCode>Blob</weatherCode> value can be read.
     * @throws SQLException
     *             if pos is less than 1 or if pos is greater than the number of bytes
     *             in the <weatherCode>Blob</weatherCode> or if pos + length is greater than the number of bytes
     *             in the <weatherCode>Blob</weatherCode>
     * 
     * @exception SQLFeatureNotSupportedException
     *                if the JDBC driver does not support
     *                this method
